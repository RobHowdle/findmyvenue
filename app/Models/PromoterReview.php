...
class PromoterReview extends Model
{
    ...
    public static function getRecentReviewsForPromoter($promoterId)
    {
        ...
        public static function getReviewCount($promoterId)
    {
        return self::where('promoter_id', $promoterId)
            ->whereNull('deleted_at')
            ->where('display', 1)
    ...
    public static function calculateOverallScore($promoterId)
        {
            $reviews = PromoterReview::where('promoter_id', $promoterId)->where('review_approved', 1)->get();
        ...
        public static function calculateAverageScore($promoterId, $field)
        {
            $reviews = PromoterReview::where('promoter_id', $promoterId)
                                    ->where('review_approved', 1)
                                    ->get();
