...
<form action="{{ route('photographer.update', ['dashboardType' => $dashboardType, 'user' => $user->id]) }}" method="POST"
  class="mt-4">
  @csrf
  @method('PUT')
  <div class="mt-4 grid sm:grid-cols-2 sm:gap-3 lg:grid-cols-3 lg:gap-4">
    @php
      $photographerGenres = is_string($photographer->genre) ? explode(',', $photographer->genre) : $photographer->genre;
    @endphp
    ...
    <div class="flex items-center">
      <input id="all-genres" name="all-genres" type="checkbox" value=""
        ...
        {{ in_array('All', $photographerGenres) ? 'checked' : '' }}>
        ...
        <div class="accordion" id="accordion-container-{{ $index }}">
          <div class="accordion-item">
            <input type="checkbox"
              ...
              {{ in_array($genre['name'], $photographerGenres) ? 'checked' : '' }}>
              ...
              @foreach ($genre['subgenres'] as $subIndex => $subgenre)
                <div class="checkbox-wrapper mb-2 flex items-center">
                  <input type="checkbox"
                    ...
                    value="{{ $subgenre }}" {{ in_array($subgenre, $photographerGenres) ? 'checked' : '' }}>
...
<script>
  ...
  let selectedGenres = "{{ json_encode($photographerGenres) }}";
