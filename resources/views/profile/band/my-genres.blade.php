...
<div class="grid sm:grid-cols-2 sm:gap-3 lg:grid-cols-3 lg:gap-4">
  @php
    $artistGenres = is_string($artist->genre) ? json_decode($artist->genre, true) : $artist->genre;
    $bandGenres = is_string($artist->band_type) ? json_decode($artist->band_type, true) : $artist->band_type;
  @endphp
...
<script defer>
  ...
  let artistGenres = @json($bandData['artistGenres']);
  let artistBandTypes = @json($bandData['bandTypes']);
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    setTimeout(() => {
      preCheckCheckboxes(artistGenres);
      preTickBandTypeCheckboxes(artistBandTypes)
    }, 500);
    updateGenreStatus();
  });

  function updateGenreStatus() {
    // Extract genres dynamically from artistGenres
    const genres = Object.keys(artistGenres);
  ...
  function preCheckCheckboxes(artistGenres) {
    if (!artistGenres || typeof artistGenres !== "object") {
      return;
    }

    Object.entries(artistGenres).forEach(([genreName, genreData], index) => {
