...
<div class="grid sm:grid-cols-2 sm:gap-3 lg:grid-cols-3 lg:gap-4">
  @php
    $standardUserGenres = is_string($standardUser->genre)
        ? json_decode($standardUser->genre, true)
        : $standardUser->genre;
    $standardUserBandType = is_string($standardUser->band_type)
        ? json_decode($standardUser->band_type, true)
        : $standardUser->band_type;
  @endphp
...
<script defer>
  ...
  let standardUserGenres = @json($standardUserData['standardUserGenres']);
  let standardUserBandType = @json($standardUserData['bandTypes']);
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    setTimeout(() => {
      preCheckCheckboxes(standardUserGenres);
      preTickBandTypeCheckboxes(standardUserBandType)
    }, 500);
    updateGenreStatus();
  });

  function updateGenreStatus() {
    // Extract genres dynamically from standardUserGenres
    const genres = Object.keys(standardUserGenres);
  ...
  function preCheckCheckboxes(standardUserGenres) {
    if (!standardUserGenres || typeof standardUserGenres !== "object") {
      console.warn("No valid genres data available for pre-checking.");
      return;
    }

    Object.entries(standardUserGenres).forEach(([genreName, genreData], index) => {
