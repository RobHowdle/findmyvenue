    ...
    <div class="relative mb-8 shadow-md sm:rounded-lg">
      <div class="min-w-screen-xl mx-auto max-w-screen-xl rounded-lg bg-yns_dark_gray px-16 py-12 text-white">
        ...
        <div class="mb-4 grid grid-cols-2 gap-x-8 gap-y-4">
          <div class="group">
            <x-input-label-dark>What is the name of your Video Company?
              <span id="result-count"></span>
            </x-input-label-dark>
            <x-text-input id="videographer-search"></x-text-input>
            <ul class="mt-2 flex flex-col gap-4 rounded-lg" id="videographer-results"></ul>
          </div>
        </div>
        ...
        <div class="mb-4">
          <div class="col-span-2" id="create-promoter-form" style="display: none;">
            ...
            <form action="{{ route('videographer.create', ['dashboardType' => $dashboardType]) }}"
              class="grid grid-cols-2 gap-x-8 gap-y-4" id="videographer-create-form" method="POST"
...
<script>
  jQuery(document).ready(function() {
    // Videographer search functionality
    jQuery('#videographer-search').on('keyup', function() {
      ...
      $.ajax({
        url: '{{ route('videographer.search', ['dashboardType' => ':dashboardType']) }}'
        ...
        success: function(data) {
          jQuery('#videographer-results').html('');
          jQuery('#result-count').text('');

          if (data.count > 0) {
            jQuery('#create-videographer-form').hide();
            jQuery('#videographer-results').html(data.results.map(videographer => `
        <li class="flex px-2 flex-row items-center justify-between">
            <p>${videographer.name}</p>
            <button class="bg-white text-black rounded-lg px-4 py-2 font-heading transition duration-150 ease-in-out hover:border-yns_yellow hover:text-yns_yellow" data-id="${videographer.id}">
                Link
            </button>
        </li>
    `).join(''));
            ...
            jQuery('#create-videographer-form').show(); // Explicitly show the form
            jQuery('#videographer-results').html('<li>No videographer found</li>');
    ...
    jQuery('#videographer-results').on('click', 'button', function() {
      const videographerId = jQuery(this).data('id'); // Retrieve the id from data-id attribute
      linkToVideographer(videographerId); // Call your function
    });

    function linkToVideographer(videographerId) {
      const dashboardType = "{{ $dashboardType }}";
      $.ajax({
